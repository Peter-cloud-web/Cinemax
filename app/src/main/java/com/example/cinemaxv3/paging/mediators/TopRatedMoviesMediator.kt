...
class TopRatedMoviesMediator @Inject constructor(
    ...
    override suspend fun initialize(): InitializeAction {
        ...
        return if (System.currentTimeMillis() - (db.getTopRatedRemoteKeysDao().getTopRatedCreationTime()?:0)<cachedTimeout){
    ...
    try{
        ...
        db.withTransaction {
            ...
            db.getTopRatedRemoteKeysDao().insertAllTopRatedKeys(remoteKeys as List<TopRatedRemoteKeys>)
            db.getTopRatedMoviesDao().insertTopRatedMovies(topRatedMovies.onEachIndexed { _, movie -> movie.page = page })
        }
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, TopRatedMovies>): TopRatedRemoteKeys? {
        return state.pages.firstOrNull {
            ...
            db.getTopRatedRemoteKeysDao().getTopRatedRemoteKeyByMovieID(movie.id)
        }
    }
